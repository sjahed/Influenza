...
struct house
{
	...
	/* yuwen add for household workplace begin */
	int    type; //household type 1 to 100
	/* yuwen add for household workplace end */
};
...
/*yuwen add random number generation begin*/
//float srandom(void); // yuwen comment generate a random number between 0 and 1
/*yuwen add random number generation end*/
...
/*yuwen delete MI feature end*/
/*yuwen add MI feature begin*/
struct  bus business[12000];
int	    number_businesses = 0; // total number of businesses in the community
/*yuwen add MI feature end*/
...
void main()
{
	...
	while(t_now < max_days*24) { // daily simulation begins ...//yuwen comment max_days is defined in "constants" file
        ...
        /*yuwen delete home 1 is not correct begin*/
		//for (k = 2; k <= number_businesses; ++k) {
		/*yuwen delete home 1 is not correct end*/
		/*yuwen add home 1 is not correct begin*/
		for (k = workplaces[1][2] + 1; k <= number_businesses; ++k) {
		/*yuwen add home 1 is not correct end*/
...
void read_input()
{
	...
	/*yuwen delete MI feature end*/
	/*yuwen add MI feature begin*/
	input1 = fopen("C:\\Users\\Yuwen\\Dropbox\\PI\\Single_city_43\\households_MI.txt", "r");
	input2 = fopen("C:\\Users\\Yuwen\\Dropbox\\PI\\Single_city_43\\workplaces_MI.txt", "r");
	input3 = fopen("C:\\Users\\Yuwen\\Dropbox\\PI\\Single_city_43\\age_child_MI.txt", "r");
	input4 = fopen("C:\\Users\\Yuwen\\Dropbox\\PI\\Single_city_43\\age_adult_MI.txt","r");
	/*yuwen add MI feature end*/
	...
	/*yuwen delete random number generation begin*/
	//for (i = 1; i <= 10000; ++i) 
	//{
		//uniform[i] = random(); //yuwen remind uniform[1] to [10000],i=10001
	//}
	/*yuwen delete random number generation end*/
		...
		for (k=1;k<=number_household;++k) { //yuwen remind k= 1 to 50000, k=50001, assign each household
			...
			for (m=1;m<=n1;++m) {
				...
				if (y < age_adult[1][2]) { //yuwen remind it's better to generate another random number, otherwise age and household type are correlated.
					community[N].age = (int)uni(23,age_adult[1][1]); //yuwen quest comment 22-28 [22,29)
				}
				if (y >= age_adult [1][2] && y < age_adult[2][2]) {
					community[N].age = (int)uni(age_adult[1][1],age_adult[2][1]); //yuwen comment 29-63 [29,64)
				}
				else {
					community[N].age = (int)uni(age_adult[2][1],age_adult[3][1]); //yuwen comment 64-98 [64,99)
				}
...
void schedule_adult_weekday(int id1)
{
	...
	if (community[id1].workplace > workplaces[1][2]) { //yuwen comment workplace from 1 to 100 is home
...
// initialize households and businesses
	...
	/* yuwen delete schedule[i] intial to home = 1 to 100 end */
	/* yuwen add schedule[i] intial to home = 1 to 100 begin */
	for (i = workplaces[1][2] + 1; i <= number_businesses; ++i) { //yuwen comment depend on the number of home in workplace table file
	/* yuwen add schedule[i] intial to home = 1 to 100 end */
	...
	for (i = 1; i <= N; ++i) {
		...
		//yuwen delete schedule[i] intial to home = 1 to 100 end 		
		//yuwen add schedule[i] intial to home = 1 to 100 begin
		if (b > workplaces[1][2] && (business[b].count_total <= business_capacity - 1)) {
		//yuwen add schedule[i] intial to home = 1 to 100 end
		...
		/* yuwen delete schedule[i] intial to home = 1 to 100 end */
		/* yuwen add schedule[i] intial to home = 1 to 100 begin */
		else if (b >= 1 && b <= workplaces[1][2] && (household[d].count_total <= household_capacity - 1)){
		/* yuwen add schedule[i] intial to home = 1 to 100 end */
		...
		/* yuwen delete schedule[i] intial to home = 1 to 100 end */
		/* yuwen add schedule[i] intial to home = 1 to 100 begin */
		/*yuwen delete wrong business_capacity upper limitation begin*/
		//else if (b > workplaces[1][2] && (business[b].count_total > business_capacity - 1)) {
		/*yuwen delete wrong business_capacity upper limitation end*/
		/*yuwen add wrong business_capacity upper limitation begin*/
		else if (b > workplaces[1][2] && (business[b].count_total > business_capacity)) {
		/*yuwen add wrong business_capacity upper limitation end*/
		/* yuwen add schedule[i] intial to home = 1 to 100 end */
			printf("Avise %d\n", b);
		}
	}
	...
	//Determine whether individuals are symptomatic or not and their  infectiousness profile
		if ((community[i].disease_clock_pandemic >= 1 && community[i].disease_clock_pandemic < culmination_period && community[i].infection_day_pandemic>0) || (community[i].disease_clock_seasonal >= 1 && community[i].disease_clock_seasonal < culmination_period && community[i].infection_day_seasonal>0)){
			...
			if (day%7 != 0 && day%7 != 6){
				...
				/* yuwen delete schedule[i] intial to home = 1 to 100 end */
				/* yuwen add schedule[i] intial to home = 1 to 100 begin */
				if (community[i].workplace > workplaces[1][2]){ //yuwen comment workplace > 100
				/* yuwen add schedule[i] intial to home = 1 to 100 end */
					//Employees
...
/**************************************************************************/
double random_backup(double min_rand, double max_rand)
{
	int i;
...
float srandom(void)
{
	...
	for (i = 1; i <= 10000; ++i)
	{
		uniform[i] = rand() / (double)(RAND_MAX);
		//printf("%f", uniform[i]);
	}  /* RAND MAX is random number within 0 to 32,767, rd is float between 0 and 1*/
}
